...
export default function Reservation() {
  ...
  const book = (key) => {
    axios.post(`/events/${id}/book`, { class: key }).then((res) => {
      Swal.fire({
    ...
    <div className="px-4 py-16 mx-auto sm:max-w-xl md:max-w-full lg:max-w-screen-xl md:px-24 lg:px-8 lg:py-20">
      ...
      <div className="grid gap-8 row-gap-5 md:row-gap-8 lg:grid-cols-3">
        {Object.keys(eventsDetails.prices).map((key) => (
          <div
            className="p-5 cursor-pointer duration-300 transform bg-white
         border-2 border-dashed rounded shadow-sm  border-purple1
          hover:-translate-y-2"
            onClick={() => book(key)}
          >
            <div className="flex items-center mb-2 text-center">
              <p className="text-lg font-bold text-center text-darkblue ">
                {key}
              </p>
            </div>
